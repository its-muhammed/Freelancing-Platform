        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div key={task._id} className="bg-white p-4 shadow-md rounded-md mb-4">
              ...
              {bidTaskId === task._id && (
                <div className="mt-4">
                  <input type="number" placeholder="Bid Amount" value={bidAmount} onChange={(e) => setBidAmount(e.target.value)} className="border px-3 py-2 rounded-md"/>
                  <input type="text" placeholder="Message" value={bidMessage} onChange={(e) => setBidMessage(e.target.value)} className="border px-3 py-2 rounded-md ml-2"/>
                  <button onClick={() => handleBidSubmit(task._id)} className="bg-blue-600 text-white px-4 py-2 rounded-md ml-2">Submit Bid</button>
                </div>
              )}
            </div>
          ))
